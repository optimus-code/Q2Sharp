	...
	public abstract class Model : Surf
	{
		...
		public virtual void Mod_LoadSubmodels( lump_t l )
		{
			...
			for ( i = 0; i < count; i++ )
			{
				...
				out_renamed.radius = RadiusFromBounds( out_renamed.mins, out_renamed.maxs );
				out_renamed.headnode = in_renamed.headnode;
				out_renamed.firstface = in_renamed.firstface;
				out_renamed.numfaces = in_renamed.numfaces;
			}
		}
		...
		public virtual void Mod_LoadFaces( lump_t l )
		{
			...
			for ( surfnum = 0; surfnum < count; surfnum++ )
			{
				...
				out_renamed = outs[surfnum];
				out_renamed.firstedge = in_renamed.firstedge;
				out_renamed.numedges = in_renamed.numedges;
				out_renamed.flags = 0;
				out_renamed.polys = null;
				planenum = in_renamed.planenum;
				side = in_renamed.side;
				if ( side != 0 )
					out_renamed.flags |= Defines.SURF_PLANEBACK;
				out_renamed.plane = loadmodel.planes[planenum];
				ti = in_renamed.texinfo;
				if ( ti < 0 || ti >= loadmodel.numtexinfo )
					Com.Error( Defines.ERR_DROP, "MOD_LoadBmodel: bad texinfo number" );
				out_renamed.texinfo = loadmodel.texinfo[ti];
				CalcSurfaceExtents( out_renamed );
				for ( i = 0; i < Defines.MAXLIGHTMAPS; i++ )
					out_renamed.styles[i] = in_renamed.styles[i];
				i = in_renamed.lightofs;
				if ( i == -1 )
					out_renamed.samples = null;
				else
				{
					...
					pointer.Mark();
					out_renamed.samples = pointer;
				}

				if ( ( out_renamed.texinfo.flags & Defines.SURF_WARP ) != 0 )
				{
					out_renamed.flags |= Defines.SURF_DRAWTURB;
					for ( i = 0; i < 2; i++ )
					{
						out_renamed.extents[i] = 16384;
						out_renamed.texturemins[i] = -8192;
					}

					GL_SubdivideSurface( out_renamed );
				}

				if ( ( out_renamed.texinfo.flags & ( Defines.SURF_SKY | Defines.SURF_TRANS33 | Defines.SURF_TRANS66 | Defines.SURF_WARP ) ) == 0 )
					GL_CreateSurfaceLightmap( out_renamed );
				if ( ( out_renamed.texinfo.flags & Defines.SURF_WARP ) == 0 )
					GL_BuildPolygonFromSurface( out_renamed );
			}
		...
		public virtual void Mod_LoadAliasModel( model_t mod, ByteBuffer buffer )
		{
			...
			{
				buffer.Get( nameBuf );
				skinNames[i] = Encoding.ASCII.GetString( nameBuf );
				var n = skinNames[i].IndexOf( '\\' );
				if ( n > -1 )
				{
		...
		static readonly Int32 MODEL_BUFFER_SIZE = 50000;
		static SingleBuffer globalModelTextureCoordBuf = Lib.NewSingleBuffer( MODEL_BUFFER_SIZE * 2 );
		static Int32Buffer globalModelVertexIndexBuf = Lib.NewInt32Buffer( MODEL_BUFFER_SIZE );
		...
		{
			System.Diagnostics.Debug.WriteLine( "AliasModels: globalVertexBuffer size " + globalModelVertexIndexBuf.Position );
		}
	}
}