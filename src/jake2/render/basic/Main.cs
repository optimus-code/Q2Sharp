...
namespace Q2Sharp.Render.Basic
{
	public abstract class Main : Base
	{
		...
		public System.Boolean qglSelectTextureSGIS = false;
		...
		public System.Boolean qglMTexCoord2fSGIS = false;
		...
		public virtual void GL_DrawParticles( Int32 num_particles )
		{
			...
			for ( Int32 j = 0, i = 0; i < num_particles; i++ )
			{
				...
				GL.Color4( ( Byte ) ( ( color >> 0 ) & 0xFF ), ( Byte ) ( ( color >> 8 ) & 0xFF ), ( Byte ) ( ( color >> 16 ) & 0xFF ), ( Byte ) ( ( color >> 24 ) & 0xFF ) );
		...
		public virtual void R_DrawParticles( )
		{
			if ( gl_ext_pointparameters.value != 0F && qglPointParameterfEXT )
			{
				...
				new Pinnable( particle_t.vertexArray.Array, ( ptr ) =>
				{
					GL.VertexPointer( 3, VertexPointerType.Float, 0, ptr );
				} );
				GL.EnableClientState( ArrayCap.ColorArray );
				new Pinnable( particle_t.GetColorAsByteBuffer().Array, ( ptr ) =>
				{
		...
		public virtual void R_SetupGL( )
		{
			Single screenaspect;
			Int32 x, x2, y2, y, w, h;
			x = ( Int32 ) Math.Floor( ( Double ) r_newrefdef.x * vid.GetWidth() / vid.GetWidth() );
			x2 = ( Int32 ) Math.Ceiling( ( Double ) ( r_newrefdef.x + r_newrefdef.width ) * vid.GetWidth() / vid.GetWidth() );
			y = ( Int32 ) Math.Floor( ( Double ) vid.GetHeight() - r_newrefdef.y * vid.GetHeight() / vid.GetHeight() );
			y2 = ( Int32 ) Math.Ceiling( ( Double ) vid.GetHeight() - ( r_newrefdef.y + r_newrefdef.height ) * vid.GetHeight() / vid.GetHeight() );
			w = x2 - x;
			h = y - y2;
			GL.Viewport( x, y2, w, h );
			screenaspect = ( Single ) r_newrefdef.width / r_newrefdef.height;
		...
		protected virtual System.Boolean R_SetMode( )
		{
			...
			fullscreen = ( vid_fullscreen.value > 0F );
		...
		public override System.Boolean R_Init2( )
		{
			...
			{
				if ( gl_ext_multitexture.value != 0F )
				{
			...
			if ( gl_config.extensions_string.IndexOf( "GL_SGIS_multitexture" ) >= 0 )
			{
				...
				else if ( gl_ext_multitexture.value != 0F )
				{
		...
		public override void R_SetPalette( Byte[] palette )
		{
			...
			if ( palette != null )
			{
				...
				for ( i = 0; i < 256; i++ )
				{
					...
					color = ( Int32 ) ( color | 0xFF000000 ); // TODO
			...
			else
			{
				for ( i = 0; i < 256; i++ )
				{
					r_rawpalette[i] = ( Int32 ) ( d_8to24table[i] | 0xff000000 );
				}
			}
		...
		public virtual void R_DrawBeam( entity_t e )
		{
			Int32 i;
			Single r, g, b;
			Single[] perpvec = new Single[] { 0, 0, 0 };
			Single[] direction = new Single[] { 0, 0, 0 };
			Single[] normalized_direction = new Single[] { 0, 0, 0 };
